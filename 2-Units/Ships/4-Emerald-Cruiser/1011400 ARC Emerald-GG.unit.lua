...
return {
	...
	data = {
		typeID       = 1011400, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	health = {
		...
		health = healthStats.baseHealth.MH * functions.averageMultiplier({healthStats.healthMult.G,healthStats.healthMult.G}),
		health_regen_per_second = healthStats.regen.MH,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.MH * functions.averageMultiplier({healthStats.armorMult.G,healthStats.armorMult.G})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.18 * functions.averageMultiplier({healthStats.accelMult.G,healthStats.accelMult.G}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
