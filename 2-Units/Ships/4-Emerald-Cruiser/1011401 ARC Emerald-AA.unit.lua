...
return {
	...
	data = {
		typeID       = 1011401, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	ghostParts 	= {
		prefab.ship.emerald.stern.A(scaleConst, true),
		prefab.ship.emerald.bow.A(scaleConst, true),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = healthStats.baseHealth.MH * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A}),
		health_regen_per_second = healthStats.regen.MH,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.MH * functions.averageMultiplier({healthStats.armorMult.A,healthStats.armorMult.A})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.18,               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		...
		standoffDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.railgun.baseRange-1,              -- float: How far away from a target's ColliderDimensions should a unit hold position?
		retreatDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.railgun.baseRange-3,               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
