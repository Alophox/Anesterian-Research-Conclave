...
return {
	...
	health = {
		...
		health = healthStats.baseHealth.MH * functions.averageMultiplier({healthStats.healthMult.C,healthStats.healthMult.C}),
		health_regen_per_second = healthStats.regen.MH,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.MH * functions.averageMultiplier({healthStats.armorMult.C,healthStats.armorMult.C})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.18 * functions.averageMultiplier({healthStats.accelMult.C,healthStats.accelMult.C}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
