shipConst = {
	class="MH",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.lightning.baseRange,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.lightning.baseRange-2,
...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-emerald-gg",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Emerald-GG",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Gunship Cruiser", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-emerald-gg.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.emerald.stern.G(),
			prefab.weapon_info.emerald.bow.G(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "MEDIUM_ARMOUR, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "MEDIUM_ARMOUR, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "MEDIUM_ARMOUR, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	data = {
		typeID       = 3291400, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.emerald.stern.G(scaleConst, false),
		prefab.ship.emerald.bow.G(scaleConst, false),
		prefab.ship.emerald.stern.G(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.emerald.bow.G(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.emerald.stern.G(scaleConst, true, 2),
		prefab.ship.emerald.bow.G(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.G,healthStats.healthMult.G})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.G,healthStats.healthMult.G})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.G,healthStats.armorMult.G})),
		vulnerability_max = 0,
		shredMultiplier = 0,
		...
		death_countdown = 3,                  	-- float: If the unit dies, but has not taken enough damage for InstaDeath, count down this long then blowup.
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.G,healthStats.accelMult.G}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.G,healthStats.retreatMult.G}),      -- float: Fraction of accel used for reverse/breaking
	...
	rotation           = {
		...
		baseRotationSpeed = 12, 		-- float: Degrees/second
