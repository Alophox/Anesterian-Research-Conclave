shipConst = {
	class="MH",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.railgun.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.railgun.baseRange-5,
}
scaleConst = 1.8;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-emerald-aa",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Emerald-AA",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Artillery Cruiser", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-emerald-aa.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		tactical                    = 	"<color=yellow>Heat Artillery</color>\n"..
										...
										"",
		description                 =
		"Ignores armour but excels against mediums.",
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.emerald.stern.A(),
			prefab.weapon_info.emerald.bow.A(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "MEDIUM_ARMOUR, HEAVY, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "MEDIUM_ARMOUR, HEAVY, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "MEDIUM_ARMOUR, HEAVY, HEAVY_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	data = {
		typeID       = 3291401, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.emerald.stern.A(scaleConst, false),
		prefab.ship.emerald.bow.A(scaleConst, false),
		prefab.ship.emerald.stern.A(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.emerald.bow.A(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.emerald.stern.A(scaleConst, true, 2),
		prefab.ship.emerald.bow.A(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.A,healthStats.armorMult.A})),
		vulnerability_max = 0,
		shredMultiplier = 0,
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.A,healthStats.accelMult.A}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
