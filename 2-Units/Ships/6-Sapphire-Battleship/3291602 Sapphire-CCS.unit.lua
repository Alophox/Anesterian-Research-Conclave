shipConst = {
	class="HC",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.laser.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.laser.baseRange-4,
}
scaleConst = 2.8;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-sapphire-ccs",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Sapphire-CCS",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Carrier Battleship", -- string: Shown just below the unit name in tooltips and databank.
	...
	hotkey                      = "6",                             -- Unity KeyCode: For buildbar. The hotkey for this unit.
	generatePicture = true;
	-- picture                     = "329-sapphire-ccs.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.sapphire.stern.C(),
			prefab.weapon_info.sapphire.core.C(),
			prefab.weapon_info.sapphire.bow.S(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        dangerDistance = 30, -- float - distance at which adaptive AIs will consider this unit to be a threat and factor it into their threat heatmaps. Pretty sure this doesn't affect the distance at which said AIs will react to it though, since that's a separate parameter that the AI itself uses
        threatValueMultiplier = 1.5, -- float - how threatening this unit is to adaptive AIs. Larger values will cause them to invest more into defending against this unit. Note that unit class also has a built-in threat multiplier, with larger unit classes being more threatening by default
    },

	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "329-3-Sapphire/Sapphire-Core-C",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291602, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.sapphire.stern.C(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.sapphire.core.C(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.sapphire.bow.S(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	-- ghostMesh		= "329-1-Ruby/Ruby-Core-C",   -- Used for build ghosts on spawners (yards).
	...
	ghostParts 	= {
		prefab.ship.sapphire.stern.C(scaleConst, true, 2),
		prefab.ship.sapphire.core.C(scaleConst, true, 2),
		prefab.ship.sapphire.bow.S(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "HEAVY",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.C,healthStats.healthMult.C,healthStats.healthMult.S})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.C,healthStats.healthMult.C,healthStats.healthMult.S})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.C,healthStats.armorMult.C,healthStats.armorMult.S})),
		vulnerability_max = 0,
		shredMultiplier = 0,
	...
	movement           = {
		...
		maximumAngleToTargetDegrees = 57.295779513082323, -- .2       -- float: Radians within in which a unit is allowed to accelerate towards a target.
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.C,healthStats.accelMult.C,healthStats.accelMult.S}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.C,healthStats.retreatMult.C,healthStats.retreatMult.S}),      -- float: Fraction of accel used for reverse/breaking
	...
	targetingData      = {
		acquisition = {
			...
			secondsPerScan = 3, 			-- float: How long to wait before reassessing what you're targeting.
			...
			maximumDistance = 30, 			-- float: Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
