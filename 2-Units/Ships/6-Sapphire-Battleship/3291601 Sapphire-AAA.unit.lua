shipConst = {
	class="HC",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.railgun.baseRange-3,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.railgun.baseRange-6,
}
scaleConst = 2.8;
return {

	-- 🟦 DEFINITIONS
	unitName                    = "329-sapphire-aaa",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Sapphire-AAA",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Artillery Battleship", -- string: Shown just below the unit name in tooltips and databank.
	...
	hotkey                      = "5",                             -- Unity KeyCode: For buildbar. The hotkey for this unit.
	generatePicture = true;
	-- picture                     = "329-sapphire-aaa.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.sapphire.stern.A(),
			prefab.weapon_info.sapphire.core.A(),
			prefab.weapon_info.sapphire.bow.A(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = {"HEAVY_ARMOURED","CAPITAL","CAPITAL_ARMOURED"}, -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = {"HEAVY","HEAVY_STANDOFF"}, -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = {"HEAVY","HEAVY_STANDOFF"}, -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "329-3-Sapphire/Sapphire-Core-A",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291601, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	-- ghostMesh		= "329-1-Ruby/Ruby-Core-A",   -- Used for build ghosts on spawners (yards).
	...
	ghostParts 	= {
		prefab.ship.sapphire.stern.A(scaleConst, true, 2),
		prefab.ship.sapphire.core.A(scaleConst, true, 2),
		prefab.ship.sapphire.bow.A(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "HEAVY",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = functions.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.A,healthStats.armorMult.A,healthStats.armorMult.A})),
		vulnerability_max = 0,
		shredMultiplier = 0,
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.A,healthStats.accelMult.A,healthStats.accelMult.A}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.A,healthStats.retreatMult.A,healthStats.retreatMult.A}),      -- float: Fraction of accel used for reverse/breaking
