...
return {
	...
	health = {
		...
		health = healthStats.baseHealth.H * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M}),
		health_regen_per_second = healthStats.regen.H,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.H * functions.averageMultiplier({healthStats.armorMult.M,healthStats.armorMult.M,healthStats.armorMult.M})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.15 * functions.averageMultiplier({healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
