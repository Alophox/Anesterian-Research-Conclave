shipConst = {
	class="H",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.missile.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.missile.baseRange-4
}
scaleConst = 2.35;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-aquamarine-mmm",                 -- string: Internal name for debugging and errors.
	...
	-- picture                     = "329-aquamarine-mmm.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		tactical                    = 	"<color=yellow>Missile Artillery</color>\n"..
										...
										" - Poor against 40+ armor / Point Defense\n"..
										" - <color=#80ffff>"..healthStats.aegisRegen[shipConst.class].." Aegis/s</color>\n"..
										"",
		description                 =
		"I heard you like missiles, so I installed all of the missiles.",
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.aquamarine.stern.M(),
			prefab.weapon_info.aquamarine.core.M(),
			prefab.weapon_info.aquamarine.bow.M(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "3-Aquamarine/Aquamarine-Core-M",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291502, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	-- ghostMesh		= "329-1-Ruby/Ruby-Core-M",   -- Used for build ghosts on spawners (yards).
	...
	ghostParts 	= {
		...
		prefab.ship.aquamarine.bow.M(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "HEAVY",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.M,healthStats.armorMult.M,healthStats.armorMult.M})),
		vulnerability_max = 0,
		shredMultiplier = 0,
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.M,healthStats.retreatMult.M,healthStats.retreatMult.M}),      -- float: Fraction of accel used for reverse/breaking
	...
	targetingData      = {
		acquisition = {
			...
			secondsPerScan = 2, 			-- float: How long to wait before reassessing what you're targeting.
