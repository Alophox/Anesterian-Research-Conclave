shipConst = {
	class="H",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.cannon.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.cannon.baseRange*.5,
}
scaleConst = 2.35;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-aquamarine-bbb",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Aquamarine-BBB",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Brawler Battlecruiser", -- string: Shown just below the unit name in tooltips and databank.
	...
	hotkey                      = "1",                             -- Unity KeyCode: For buildbar. The hotkey for this unit.
	generatePicture = true;
	-- picture                     = "329-aquamarine-bbb.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			...
			prefab.weapon_info.aquamarine.bow.B(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, LIGHT_ARMOUR, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        dangerDistance = 25, -- float - distance at which adaptive AIs will consider this unit to be a threat and factor it into their threat heatmaps. Pretty sure this doesn't affect the distance at which said AIs will react to it though, since that's a separate parameter that the AI itself uses
	...
	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "3-Aquamarine/Aquamarine-Core-B",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291501, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.aquamarine.stern.B(scaleConst, false),
		prefab.ship.aquamarine.core.B(scaleConst, false),
		prefab.ship.aquamarine.bow.B(scaleConst, false),
		prefab.ship.aquamarine.stern.B(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.aquamarine.core.B(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.aquamarine.bow.B(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	-- ghostMesh		= "329-1-Ruby/Ruby-Core-B",   -- Used for build ghosts on spawners (yards).
	...
	ghostParts 	= {
		prefab.ship.aquamarine.stern.B(scaleConst, true, 2),
		prefab.ship.aquamarine.core.B(scaleConst, true, 2),
		prefab.ship.aquamarine.bow.B(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "HEAVY",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.B,healthStats.armorMult.B,healthStats.armorMult.B})),
		vulnerability_max = 0,
		shredMultiplier = 0,
		...
		death_countdown = 4,                  	-- float: If the unit dies, but has not taken enough damage for InstaDeath, count down this long then blowup.
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.B,healthStats.accelMult.B,healthStats.accelMult.B}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.B,healthStats.retreatMult.B,healthStats.retreatMult.B}),      -- float: Fraction of accel used for reverse/breaking
	...
	rotation           = {
		...
		baseRotationSpeed = 10, 		-- float: Degrees/second
