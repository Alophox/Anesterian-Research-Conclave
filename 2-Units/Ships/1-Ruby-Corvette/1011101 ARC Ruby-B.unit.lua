...
return {
	...
	data = {
		typeID       = 1011101, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	health = {
		...
		health = healthStats.baseHealth.L * functions.averageMultiplier({healthStats.healthMult.B}),
		health_regen_per_second = healthStats.regen.L,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.L * functions.averageMultiplier({healthStats.armorMult.B})),
