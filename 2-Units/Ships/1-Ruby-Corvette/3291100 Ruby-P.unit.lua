...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-ruby-p",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Ruby-P",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	...
	-- picture                     = "329-ruby-p.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
				prefab.weapon_info.ruby.core.P(),
		}), 	-- int3 array: Tells the databank which weaponDatas to grab and display for this unit. Not automatic you have to do this, I'm sorry.
  		-- (int, int, int): weaponID, count, subMunitionWeaponID (bullet, missile, etc, 0 = none) This gets the damage data of the subMunitionWeapon if it has any, important for bullets which themselves do damage via a subweapon.
    ...
    unitMetaRole = {
        ...
        offenceRole = "MISSILE, DRONE, DRONE_ARMOURED, LIGHT", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "MISSILE, DRONE, DRONE_ARMOURED, LIGHT", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "MISSILE, DRONE, DRONE_ARMOURED, LIGHT", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	-- mainMesh                    	= "329-1-Ruby/Ruby-Core-P",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291100, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.ruby.core.P(scaleConst, false),
		prefab.ship.ruby.core.P(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	-- ghostMaterials = { ghostMat, ghostMat, ghostMat,ghostMat }, -- Used for the ghostMesh for build ghosts on spawners (yards).
	ghostParts 	= {
		prefab.ship.ruby.core.P(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "LIGHT",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.P})),              -- float: Health, also the unit's heat capacity.
		health_regen_per_second = 0,
		aegisMaximum = math.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.P})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,
		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.P})),                -- int: Reduces incoming damage. Used to allow heavier ship classes to withstand many smaller opponents, but still being countered by anti-armour. Lights ~5, Mediums ~10, Heavies ~20, Capitals ~50
	...
	movement           = {
		...
		standoffDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.laser.baseRange-2,              -- float: How far away from a target's ColliderDimensions should a unit hold position?
		retreatDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.laser.baseRange - 3,               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
	...
	rotation           = {
		...
		baseRotationSpeed = 60, 		-- float: Degrees/second
	...
	targetingData      = {
		acquisition = {
			...
			classMultHeavy = 0.8,	-- float: 
			classMultCapital = 0.8,	-- float: 
			classMultTitan = 0.8,	-- float: 
