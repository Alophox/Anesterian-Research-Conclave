...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-citrine-a",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Citrine-A",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Artillery Frigate", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-citrine-a.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		.."\n\n"
		.."However, due to the frequency of maintenance required to keep such a large weapon functional on a lighter vessel, it was swapped for many smaller ones on the automated versions- as fixing a weapon mid-battle is not feasible when you have no crew.",
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.citrine.core.A(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        dangerDistance = 15, -- float - distance at which adaptive AIs will consider this unit to be a threat and factor it into their threat heatmaps. Pretty sure this doesn't affect the distance at which said AIs will react to it though, since that's a separate parameter that the AI itself uses
	...
	-- mainMesh                    	= "329-2-Citrine/Citrine-Core-A",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	colliderDimensions = {
		widthMultiplier  = 0.418, -- float: Multiplied by scale to determine the percieved width of the unit. Long units (Vaalkorei) have this at about 0.5~0.6, aka we're only half as wide as we are long.
		heightMultiplier = 0.294, -- float: Multiplied by scale to determine the percieved height of the unit, flat units (Kontaalen) have this at about 0.6, aka we're shorter than we are long.
	...
	data = {
		typeID       = 3291201, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	-- ghostMesh		= "329-2-Citrine/Citrine-Core-A",   -- Used for build ghosts on spawners (yards).
	-- ghostMaterials = { ghostMat, ghostMat, ghostMat, ghostMat,ghostMat }, -- Used for the ghostMesh for build ghosts on spawners (yards).
	ghostParts 	= {
		prefab.ship.citrine.core.A(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "LIGHT",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.A})),
		vulnerability_max = 0,
		shredMultiplier = 0,
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.A}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.A}),      -- float: Fraction of accel used for reverse/breaking
		...
		standoffDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.railgun.baseRange-2,              -- float: How far away from a target's ColliderDimensions should a unit hold position?
		retreatDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.railgun.baseRange-5,               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
	...
	rotation           = {
		...
		baseRotationSpeed = 30, 		-- float: Degrees/second
