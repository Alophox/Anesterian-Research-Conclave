...
return {
	...
	health = {
		...
		health = healthStats.baseHealth.LM * functions.averageMultiplier({healthStats.healthMult.G}),
		health_regen_per_second = healthStats.regen.LM,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.LM * functions.averageMultiplier({healthStats.armorMult.G})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.27 * functions.averageMultiplier({healthStats.accelMult.G}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
