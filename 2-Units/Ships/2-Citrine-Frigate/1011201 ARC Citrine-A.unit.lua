...
return {
	...
	data = {
		typeID       = 1011201, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	health = {
		...
		health = healthStats.baseHealth.LM * functions.averageMultiplier({healthStats.healthMult.A}),
		health_regen_per_second = healthStats.regen.LM,
		max_regen_frac = healthStats.proportionRegenMax,

		armour = functions.floor(healthStats.baseArmor.LM * functions.averageMultiplier({healthStats.armorMult.A})),
		vulnerability_max = 0,
		shredMultiplier = 1.0,
	...
	movement           = {
		...
		acceleration = 0.27 * functions.averageMultiplier({healthStats.accelMult.A}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
