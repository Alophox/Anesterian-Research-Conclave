...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-zircon",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Zircon",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Artillery Drone", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-zircon.png",             -- string filename: The name of the image file in this folder to be used for this unit.
    ...
    unitMetaRole = {
        ...
        offenceRole = "DRONE, LIGHT, LIGHT_ARMOUR, MEDIUM_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "DRONE, LIGHT, LIGHT_ARMOUR, MEDIUM_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "DRONE, LIGHT, LIGHT_ARMOUR, MEDIUM_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        dangerDistance = 10, -- float - distance at which adaptive AIs will consider this unit to be a threat and factor it into their threat heatmaps. Pretty sure this doesn't affect the distance at which said AIs will react to it though, since that's a separate parameter that the AI itself uses
	...
	colliderDimensions = {
		widthMultiplier  = 0.6, -- float: Multiplied by scale to determine the percieved width of the unit. Long units (Vaalkorei) have this at about 0.5~0.6, aka we're only half as wide as we are long.
		heightMultiplier = 0.5, -- float: Multiplied by scale to determine the percieved height of the unit, flat units (Kontaalen) have this at about 0.6, aka we're shorter than we are long.
	...
	data = {
		typeID       = 3291005, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	health = {
		...
		untargetableTime = 0,    -- float: Useful on drones/missiles. How long after spawning is this unit untargetable? (0 for everything, except missiles and drones)
	...
	isNotRepulsed        = false,
	repulsion            = {
		interactionLayer = 1,    -- Units will only repulse if they use the same layer. Ships 0, Drones 1
	...
	movement           = {
		...
		maximumAngleToTargetDegrees = 89.954373835539243,        -- float: Radians within in which a unit is allowed to accelerate towards a target.
		...
		standoffDistance = math.floor(.5+weaponStats.fireRangeMult * weaponStats.rangeMult.D * weaponStats.railgun.baseRange-1),              -- float: How far away from a target's ColliderDimensions should a unit hold position?
		retreatDistance = math.floor(.5+weaponStats.fireRangeMult * weaponStats.rangeMult.D * weaponStats.railgun.baseRange-2),               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
	...
	targetingData      = {
		acquisition = {
			...
			secondsPerScan = .5, 			-- float: How long to wait before reassessing what you're targeting.
			...
			perceivedDistanceMin = 1, 		-- float: Clamps the score for distance. Good in general. Keep at a fraction of your maximumDistance. Once a target is within normal firing range it's good to keep the target and not prioritise other things just because they're closer.

			-- Target class priority.
			-- Vital for unit behaviour. -1 = ignore.
			-- Multiplies the target score by this.
			classMultMissile = -1,	-- float: 
			classMultDrone = .2,	-- float: 
			classMultLight = 1,		-- float: 
			classMultMedium = 1,	-- float: 
			classMultHeavy = 0.9,	-- float: 
			classMultCapital = 0.8,	-- float: 
			classMultTitan = 0.7,	-- float: 
		...
		tracking = {
			...
			predictionVelocityOveride = weaponStats.railgun.velocity, 	-- float: The speed of the thing we want to hit our target with. For bullets use weapon unitsPerSecond. EG. An Untresnafol has this set to the launch velocity of it's bullet.
