...
return {
	...
	unitBlurb                   = "Bomber", -- string: Shown just below the unit name in tooltips and databank.
	...
	mainMesh                    	= "0-Quartz/Quartz",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	parts = {
		...
		{
			name     = "Jank Targeting Base 1",
			position = { 0, 0, 0 },
			rotation = { 0, 0, 0 },
			scale 	= { 1, 1, 1 },
			weapon    = {
				weaponID = 3299101, --int: The weaponData id to be used for this weapon.
					...
					parts={
						{
							...
							scale 	 = { 1, 1, 1 },
							weapon    = {
								weaponID = 3299102, --int: The weaponData id to be used for this weapon.
								turnSpeed = 240, 	--float: Degrees per second.
								turnMode = "Linear", --string enum: Linear / Acceleration
								turnInstant = false, --bool: Ignore turn speed, snap to target. (Beam Spire, point defence)
								mountAngles = { -- Weapon's firing angles in degrees. Won't aquire targets outside this field of view.
									left = 180, --float:
									...
									parts = {
										{
											...
											mesh       = "Rift/Rift",
											materials  = { "arc_hull" },
											position   = { 0, 0, -.1/scaleConst},
											rotation   = { 0, 0, 0 },
											scale 	 = { .1, .1, .1 },
											autoModule = {
												moduleTypeID = 3299701,            -- int
	...
	movement           = {
		...
		retreatDistance = -1,               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
	...
	rotation           = {
		...
		baseRotationSpeed = 100, 		-- float: Degrees/second
	...
	targetingData      = {
		acquisition = {
			...
			pauseIfHasTarget = true, 		--Don't look for a target again, until you're current one is dead. USE WITH canInvalidateTarget otherwise units WILL get stuck trying to shoot stuff out of their range.
