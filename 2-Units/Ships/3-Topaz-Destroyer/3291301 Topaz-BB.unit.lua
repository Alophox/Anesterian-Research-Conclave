shipConst = {
	class="M",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.cannon.baseRange,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["S"] * weaponStats.cannon.baseRange-2,
}
scaleConst = 1.5;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-topaz-bb",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Topaz-BB",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Brawler Destroyer", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-topaz-bb.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "329-3-Topaz/Topaz-Core-B",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291301, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.topaz.stern.B(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.topaz.bow.B(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.topaz.stern.B(scaleConst, true, 2),
		prefab.ship.topaz.bow.B(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.B,healthStats.armorMult.B})),
		vulnerability_max = 0,
		shredMultiplier = 0,
