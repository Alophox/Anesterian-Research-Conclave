shipConst = {
	class="M",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.missile.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.missile.baseRange-5,
}
scaleConst = 1.5;
return {
    isCloaking = false,
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-topaz-mm",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Topaz-MM",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Missile Destroyer", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-topaz-mm.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.topaz.stern.M(),
			prefab.weapon_info.topaz.bow.M(),
		}),
    ...
    unitMetaRole = {
        ...
        defenceResponse = "LIGHT, MEDIUM", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, MEDIUM", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	colliderDimensions = {
		widthMultiplier  = 0.3,
		heightMultiplier = 0.233,
		lengthMultiplier = 1.0,
	},

	-- 🟦 UNIT ID, STRUCTURE COST, MACROTARGET STATE, TECH
	data = {
		typeID       = 3291302, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	ghostParts 	= {
		prefab.ship.topaz.stern.M(scaleConst, true, 2),
		prefab.ship.topaz.bow.M(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor((1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M})),
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.M,healthStats.armorMult.M})),
		vulnerability_max = 0,
		shredMultiplier = 0,
		...
		death_countdown = 2,                  	-- float: If the unit dies, but has not taken enough damage for InstaDeath, count down this long then blowup.
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.M,healthStats.accelMult.M}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.M,healthStats.retreatMult.M}),      -- float: Fraction of accel used for reverse/breaking
