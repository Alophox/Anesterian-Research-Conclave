...
return {
	...
	ghostParts 	= {
		prefab.ship.topaz.stern.M(scaleConst, true),
		prefab.ship.topaz.bow.M(scaleConst, true),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "MEDIUM",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M}),
	...
	movement           = {
		...
		retreatDistance = weaponStats.fireRangeMult * weaponStats.rangeMult["M"] * weaponStats.missile.baseRange-4,               -- float: How far away from a target's ColliderDimensions should a unit begin to retreat?
