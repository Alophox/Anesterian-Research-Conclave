...
return {
	...
	ghostParts 	= {
		prefab.ship.amethyst.stern.M(scaleConst, true),
		prefab.ship.amethyst.core.M(scaleConst, true),
		prefab.ship.amethyst.bow.M(scaleConst, true),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "CAPITAL",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health =  healthStats.baseHealth.C * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M}),              -- float: Health, also the unit's heat capacity.
		...
		armour = functions.floor(healthStats.baseArmor.C * functions.averageMultiplier({healthStats.armorMult.M,healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M})),
	...
	movement           = {
		...
		acceleration = 0.05 * functions.averageMultiplier({healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
