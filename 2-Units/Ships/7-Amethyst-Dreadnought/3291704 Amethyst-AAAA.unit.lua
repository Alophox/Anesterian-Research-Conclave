shipConst = {
	class="C",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.railgun.baseRange-4,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.railgun.baseRange-7,
}
...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-amethyst-aaaa",                 -- string: Internal name for debugging and errors.
	...
	unitBlurb                   = "Artillery Dreadnought", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-amethyst-aaaa.png",             -- string filename: The name of the image file in this folder to be used for this unit.
    ...
    unitMetaRole = {
        ...
        offenceRole = "HEAVY_ARMOUR, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "HEAVY_ARMOUR, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	parts = {
		...
		prefab.ship.amethyst.stern.A(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.core.A(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.bow.A(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.amethyst.stern.A(scaleConst, true, 2),
		prefab.ship.amethyst.core.A(scaleConst, true, 2),
		prefab.ship.amethyst.bow.A(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "CAPITAL",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor( (1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A})),              -- float: Health, also the unit's heat capacity.
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A,healthStats.healthMult.A})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.A,healthStats.armorMult.A,healthStats.armorMult.A,healthStats.armorMult.A})),
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.A,healthStats.accelMult.A,healthStats.accelMult.A,healthStats.accelMult.A}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
