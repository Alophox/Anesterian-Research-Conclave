shipConst = {
	class="C",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.missile.baseRange-3,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["X"] * weaponStats.missile.baseRange-5,
}
...
return {
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.amethyst.stern.M(),
			prefab.weapon_info.amethyst.core.M(),
			prefab.weapon_info.amethyst.bow.M(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY_STANDOFF", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	data = {
		typeID       = 3291702, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.amethyst.stern.M(scaleConst, false),
		prefab.ship.amethyst.core.M(scaleConst, false),
		prefab.ship.amethyst.bow.M(scaleConst, false),
		prefab.ship.amethyst.stern.M(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.core.M(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.bow.M(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.amethyst.stern.M(scaleConst, true, 2),
		prefab.ship.amethyst.core.M(scaleConst, true, 2),
		prefab.ship.amethyst.bow.M(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "CAPITAL",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor( (1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M})),              -- float: Health, also the unit's heat capacity.
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M,healthStats.healthMult.M})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.M,healthStats.armorMult.M,healthStats.armorMult.M,healthStats.armorMult.M})),
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M,healthStats.accelMult.M}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.M,healthStats.retreatMult.M,healthStats.retreatMult.M,healthStats.retreatMult.M}),      -- float: Fraction of accel used for reverse/breaking
		...
		standoffDistance = shipConst.standoff,
		retreatDistance = shipConst.retreat,
