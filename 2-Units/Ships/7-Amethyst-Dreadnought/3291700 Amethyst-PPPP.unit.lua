shipConst = {
	class="C",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.laser.baseRange-2,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.laser.baseRange-5,
}
...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-amethyst-pppp",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Amethyst-PPPP",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Picket Dreadnought", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-amethyst-pppp.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.amethyst.stern.P(),
			prefab.weapon_info.amethyst.core.P(),
			prefab.weapon_info.amethyst.bow.P(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        dangerDistance = 35, -- float - distance at which adaptive AIs will consider this unit to be a threat and factor it into their threat heatmaps. Pretty sure this doesn't affect the distance at which said AIs will react to it though, since that's a separate parameter that the AI itself uses
	...
	-- BODY SETUP
	scale = scaleConst,
	-- mainMesh                    	= "329-3-Sapphire/Sapphire-Core-G",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3291700, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.amethyst.stern.P(scaleConst, false),
		prefab.ship.amethyst.core.P(scaleConst, false),
		prefab.ship.amethyst.bow.P(scaleConst, false),
		prefab.ship.amethyst.stern.P(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.core.P(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.bow.P(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	-- ghostMesh		= "329-1-Ruby/Ruby-Core-G",   -- Used for build ghosts on spawners (yards).
	...
	ghostParts 	= {
		prefab.ship.amethyst.stern.P(scaleConst, true, 2),
		prefab.ship.amethyst.core.P(scaleConst, true, 2),
		prefab.ship.amethyst.bow.P(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "CAPITAL",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor( (1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.P,healthStats.healthMult.P,healthStats.healthMult.P,healthStats.healthMult.P})),              -- float: Health, also the unit's heat capacity.
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.P,healthStats.healthMult.P,healthStats.healthMult.P,healthStats.healthMult.P})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.P,healthStats.armorMult.P,healthStats.armorMult.P,healthStats.armorMult.P})),
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.P,healthStats.accelMult.P,healthStats.accelMult.P,healthStats.accelMult.P}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.P,healthStats.retreatMult.P,healthStats.retreatMult.P,healthStats.retreatMult.P}),      -- float: Fraction of accel used for reverse/breaking
