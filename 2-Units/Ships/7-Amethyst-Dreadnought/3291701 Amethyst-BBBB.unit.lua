shipConst = {
	class="C",
	standoff=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.cannon.baseRange-1,
	retreat=weaponStats.fireRangeMult * weaponStats.rangeMult["L"] * weaponStats.cannon.baseRange/2,
}
...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-amethyst-bbbb",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Amethyst-BBBB",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "", 						   -- string: Shown when moused over in the HUD.
	unitBlurb                   = "Brawler Dreadnought", -- string: Shown just below the unit name in tooltips and databank.
	...
	-- picture                     = "329-amethyst-bbbb.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = functions.combineWeaponInfo({
			prefab.weapon_info.amethyst.stern.B(),
			prefab.weapon_info.amethyst.core.B(),
			prefab.weapon_info.amethyst.bow.B(),
		}),
    ...
    unitMetaRole = {
        ...
        offenceRole = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what adaptive AIs think this unit is good at attacking. Multiple roles may be assigned
        defenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what defenses adaptive AIs think they should counter this unit with. Multiple roles may be assigned
        offenceResponse = "LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY", -- enum UNITROLE - IGNORE, MISSILE, DRONE, DRONE_ARMOURED, LIGHT, LIGHT_ARMOUR, MEDIUM, MEDIUM_ARMOUR, MEDIUM_STANDOFF, HEAVY, HEAVY_ARMOUR, HEAVY_STANDOFF, CAPITAL, CAPITAL_ARMOUR, CAPITAL_STANDOFF, TITAN. Determines what ships adaptive AIs think they should counter this unit with. Multiple roles may be assigned
	...
	data = {
		typeID       = 3291701, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		prefab.ship.amethyst.stern.B(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.core.B(scaleConst,true, 0, "aegis_teamColour"),
		prefab.ship.amethyst.bow.B(scaleConst,true, 0, "aegis_teamColour"),
	},
	...
	ghostParts 	= {
		prefab.ship.amethyst.stern.B(scaleConst, true, 2),
		prefab.ship.amethyst.core.B(scaleConst, true, 2),
		prefab.ship.amethyst.bow.B(scaleConst, true, 2),
	},

	-- 🟦 HEALTH & ARMOR
	health = {
		unitClass = "CAPITAL",       -- string enum: UNITCLASS: NONE, MISSILE, DRONE, LIGHT, MEDIUM, HEAVY, CAPITAL, TITAN
		health = math.floor( (1 - healthStats.proportionRegenMax) * healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B})),              -- float: Health, also the unit's heat capacity.
		health_regen_per_second = 0,
		aegisMaximum = functions.ceil(healthStats.proportionRegenMax *  healthStats.baseHealth[shipConst.class] * functions.averageMultiplier({healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B,healthStats.healthMult.B})), 			-- float: Game will automatically determine, but can be manually set here.
		aegis_regen_per_second = healthStats.aegisRegen[shipConst.class],
		max_regen_frac = healthStats.proportionRegenMax,

		armour = math.floor(healthStats.baseArmor[shipConst.class] * functions.averageMultiplier({healthStats.armorMult.B,healthStats.armorMult.B,healthStats.armorMult.B,healthStats.armorMult.B})),
	...
	movement           = {
		...
		acceleration = healthStats.baseAccel[shipConst.class] * functions.averageMultiplier({healthStats.accelMult.B,healthStats.accelMult.B,healthStats.accelMult.B,healthStats.accelMult.B}),               -- float: The units per second of the ship's acceleration. A tolly is 0.4 units long, and accelerates at 0.35
		strafingAccelMultiplier = 0.8,     -- float: Fraction of accel used for strafing
		reverseAccelMultiplier = functions.averageMultiplier({healthStats.retreatMult.B,healthStats.retreatMult.B,healthStats.retreatMult.B,healthStats.retreatMult.B}),      -- float: Fraction of accel used for reverse/breaking
