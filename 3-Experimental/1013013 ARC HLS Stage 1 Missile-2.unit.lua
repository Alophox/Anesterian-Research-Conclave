...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "arc_lagopus",                 -- string: Internal name for debugging and errors.
	...
	picture                     = "arc_lagopus.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = { 	-- int3 array: Tells the databank which weaponDatas to grab and display for this unit. Not automatic you have to do this, I'm sorry.
			{ 1016002, 1, 0 }   		-- (int, int, int): weaponID, count, subMunitionWeaponID (bullet, missile, etc, 0 = none) This gets the damage data of the subMunitionWeapon if it has any, important for bullets which themselves do damage via a subweapon.
		},
		relatedUnitIDs              = {} 	-- int array: TypeID of other units in the family tree
	},

	-- BODY SETUP
	scale                       	= .0983334,                                          -- float: A Tolly is scale 0.4, all units are a uniform scale. Normalise XY mesh size to 1 in Blender. (controls selection ring, among other things. the longest horizontal part (width/length) is 1)
	...
	colliderDimensions = {
		widthMultiplier  = 0.39, -- float: Multiplied by scale to determine the percieved width of the unit. Long units (Vaalkorei) have this at about 0.5~0.6, aka we're only half as wide as we are long.
		heightMultiplier = 0.39, -- float: Multiplied by scale to determine the percieved height of the unit, flat units (Kontaalen) have this at about 0.6, aka we're shorter than we are long.
	...
	data = {
		typeID       = 1013013, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		...
		},
		{
			...
			weapon    = {
				weaponID = 1013014, --int: The weaponData id to be used for this weapon.
		...
		-- 	scale 	= { 1, 1, 1 },
		-- 	autoModule = {
		-- 		moduleTypeID = 1013001, -- The TypeID of the unit that will be attached to this unit, as a "module".
	...
	health = {
		...
		invulnerable = true,                 	-- You can't hurt me, little man.
	...
	movement           = {
		...
		maximumAngleToTarget = .5,        -- float: Radians within in which a unit is allowed to accelerate towards a target.
	...
	targetingData      = {
		acquisition = {
			...
			maximumDistance = 12, 			-- float: Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
			...
			scoreBandingSize = 1,			-- float: Allows the targeter to pick a random target within bands. Useful for PD and AOE, but requires a high keep target to prevent schizophrenia.
		},
		tracking = {
