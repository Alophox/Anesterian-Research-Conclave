return {
	...
	weapon = {
		...
		unitsPerSecond = .1,           -- projectile velocity for LAUNCHER in 100m/s
		spreadDegrees = 1,          -- Radians, spread for LAUNCHER (machine guns, etc)
		...
		damageData = {
			...
			instances = 0,                     -- int: How many times is this damage dealt. For making weapons worse against armour.
		...
		secondsPerCycle = .5,        --Seconds per reload. Negative value prevent reloading (limited ammo weapons).
	...
	targeting = {
		acquisition = {
			...
			maximumDistance = .2, 			--Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
			...
			scoreForHealth = -.1,      		--Niche parameter, prioritise the biggest things, or smallest if negative.
			...
			structureMultiplier = 0, 		--Priority for structures.
			keepTargetMultiplier = 0, 		--Important. Allows the unit to keep it's current target, and not bounce between things.
