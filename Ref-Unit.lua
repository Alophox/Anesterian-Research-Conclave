...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "",       -- string: Internal name for debugging and errors.
	unitDisplayName             = "",       -- string: Actual display name of the unit in the Databank, HUD, etc.
	unitTooltip                 = "",       -- string: Shown when moused over in the HUD.
	unitBlurb                   = "",       -- string: Shown just below the unit name in tooltips and databank.
	...
	picture                     = "",       -- string filename: The name of the image file in this folder to be used for this unit.
	...
	-- 📘 DATABANK ENTRY
	databankEntry = {
		visibility                  = "", -- string enum: VISIBLE (always available) / HIDDEN (never visible)/ DISCOVER (visible once required level)
		...
		description                 = "", -- string markdown: Databank description
		weaponInfo                  = { 	-- int3 array: Tells the databank which weaponDatas to grab and display for this unit. Not automatic you have to do this, I'm sorry.
			{ 0, 0, 0 },   		-- {int, int, int}: weaponID, count, subMunitionWeaponID (bullet, missile, etc, 0 = none) This gets the damage data of the subMunitionWeapon if it has any, important for bullets which themselves do damage via a subweapon.
			-- subMunitionWeaponID = -2 to fetch PAYLOAD information for the SPAWNED unit for the weaponID
			-- -1, 0, 0 to get payload information for THIS unit
		},
		relatedUnitIDs              = {} 	-- int array: TypeID of other units in the family tree
	},

	-- BODY SETUP
	scale       = .4, -- float: A Tolly is scale 0.4, all units are a uniform scale. Normalise XY mesh size to 1 in Blender. (controls selection ring, among other things. the longest horizontal part (width/length) is 1)
	mainMesh    = "", -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 0, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
		factionID    = 0, -- int: The faction this unit is associated with in the Databank.
	...
	parts = {
		{
			...
			scale 	= { 0, 0, 0 },		-- float3: XYZ The nonuniform scale of the part, relative to it's parent's scale.
			
			--optional
			weapon    = {
				weaponID = 0,	--int: The weaponData id to be used for this weapon.
				turnSpeed = 0,	--float: Degrees per second.
				turnMode = "",	--string enum: Linear / Acceleration
				...
				mountAngles = { -- Weapon's firing angles in degrees. Won't aquire targets outside this field of view.
					left = 0, 	--float: 0-180 degrees left from the resting position
					right = 0,	--float: 0-180 degrees right from the resting position
					up = 0,	 	--float: 0-90 degrees up from the resting position
					down = 0  	--float: 0-90 degrees down from the resting position
				},
			},

			-- optional; must have a weapon parent
			...
			weaponVisualConfig = { --Controls how the visual effect behaves.
				...
				laserColour = {1,.5,1}, 	-- RGB, 0 to 1, colour of the weaponVisual. Modifies the matierla's color but not its emission
			...
			parts={} --you can keep nesting parts; their scale 1 will be relative to their parent's scale
		},
	...
	ghostMesh		= "",   -- Used for build ghosts on spawners (yards).
	ghostMaterials = { "", "", "","" }, -- Used for the ghostMesh for build ghosts on spawners (yards).
	ghostParts 	= {
		--same as parts... HOWEVER, DO NOT PUT WEAPONS HERE, as that will crash the game
	},
	...
	health = {
		...
		shredMultiplier = 1.0,     -- float: Multiplies incoming shred, pretty self explanatory right?
	...
	isRepulsor		= true, --bool: does this repulse
	isNotRepulsed	= false, --boo: is this effected by repulsion
	repulsion		= {
