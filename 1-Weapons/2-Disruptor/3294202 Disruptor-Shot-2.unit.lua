...
return {
	...
	-- 🟦 DEFINITIONS
	unitName                    = "329-disruptor-shot",                 -- string: Internal name for debugging and errors.
	unitDisplayName             = "Medium Disruptor Shot",                         -- string: Actual display name of the unit in the Databank, HUD, etc.
	...
	-- picture                     = "329-disruptor-shot.png",             -- string filename: The name of the image file in this folder to be used for this unit.
	...
	databankEntry               = {
		...
		weaponInfo                  = { 	-- int3 array: Tells the databank which weaponDatas to grab and display for this unit. Not automatic you have to do this, I'm sorry.
			{ 3296202, 1, 0 }   		-- (int, int, int): weaponID, count, subMunitionWeaponID (bullet, missile, etc, 0 = none) This gets the damage data of the subMunitionWeapon if it has any, important for bullets which themselves do damage via a subweapon.
		},
		relatedUnitIDs              = {} 	-- int array: TypeID of other units in the family tree
	},

	-- BODY SETUP
	scale                       	= .3,                                          -- float: A Tolly is scale 0.4, all units are a uniform scale. Normalise XY mesh size to 1 in Blender. (controls selection ring, among other things. the longest horizontal part (width/length) is 1)
	mainMesh                    	= "329-Turrets-1/Turret-1-Disruptor-Effect",   -- string: Visual body of this unit. Requires materials to be visible. FileName/ObjectName, looks for FileName.glb and then ObjectName from within that.
	...
	data = {
		typeID       = 3294202, -- int: !!! IMPORTANT !!! The unique id of this unit. Must be higher than 99999 (ATS reserved). Used by maps and many things. If you change this any maps made with it won't be able to find the unit and will just spawn nothing.
	...
	parts = {
		{
			...
			weapon    = {
				weaponID = 3296202, --int: The weaponData id to be used for this weapon.
	...
	ghostMesh		= "329-Turrets-1/Turret-1-Disruptor-Effect",   -- Used for build ghosts on spawners (yards).
