...
return {
	...
	weapon = {
		...
		damageData = {
			...
			piercing = 999999999999999999,                      -- float: Ignore this much armour. Negative piercing is AEGIS shielding.
		...
		--Audio
		sfxID = weaponStats.disruptor.sfxID,               --int, id of the audio to play when this weapon fires.
		...
		laserDescription = { 		--"laser" description, but is actually a maleable visual effect.
						duration = .1,			--seconds
						opacity = 1,			--float, 0-1
						diameter = .3,			--relative to turret scale
						...
						noRescaleLength = -.3,	--0 means DO rescale like a laser beam. >0 means don't, like a muzzle flashes.
		...
		secondsPerCycle = weaponStats.disruptor.baseCD * weaponStats.CDMult[weaponConst.size] + weaponStats.CDMod[weaponConst.size] - weaponStats.disruptor.secondsPerShot*(weaponStats.disruptor.shotsPerBurst[weaponConst.size] - 1),        --Seconds per reload. Negative value prevent reloading (limited ammo weapons).
		secondsPerCycleCurrent = 0,   --Starting delay. Good if you don't want your bomber launching bombs the moment it spawns.

		--Magazine Size
		shotsPerCycle = weaponStats.disruptor.shotsPerBurst[weaponConst.size],            --Shots per reload, ATS will ensure this number is always at least 1.
		shotsPerCycleCurrent = weaponStats.disruptor.shotsPerBurst[weaponConst.size],     --Starting shots in the clip.
		simultaniousShots = 1,        --How many shots we can make at once. (shotgun, cluster bomb)

		--Fire Rate
		secondsPerShot = weaponStats.disruptor.secondsPerShot,           --Delay between shots.
	...
	targeting = {
		acquisition = {
			...
			maximumDistance = weaponStats.fireRangeMult * weaponStats.rangeMult[weaponConst.size] * weaponStats.disruptor.baseRange, 			--Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
			maximumAngleDegrees = 20626.480624709635, 				--Good for spinal weapons/missiles.
		...
		tracking = {
			...
			predictionVelocityOveride = weaponStats.disruptor.velocity, 	--The speed of the thing we want to hit our target with. For bullets use unitsPerSecond. EG. An Untresnafol has this set to the launch velocity of it's bullet.
