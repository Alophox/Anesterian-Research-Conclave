return {
	...
	weapon = {
		...
		rangeInUnits = 40,            -- Target must be within range, for gun to fire.
		...
		damageData = {
			...
			damage = .001,                       -- float: Raw damage, reduced by armour.
		...
		laserDescription = { 		--"laser" description, but is actually a maleable visual effect.
						...
						noRescaleLength = 2,	--0 means DO rescale like a laser beam. >0 means don't, like a muzzle flashes.
		...
		secondsPerCycle = 10,        --Seconds per reload. Negative value prevent reloading (limited ammo weapons).
	...
	targeting = {
		acquisition = {
			...
			maximumDistance = 40, 			--Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
			...
			scoreBandingSize = 5,			--Allows the targeter to pick a random target within bands. Useful for PD and AOE, but requires a high keep target to prevent schizophrenia.
		},
		tracking = {
