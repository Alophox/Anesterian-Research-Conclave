return {
	...
	blurb = "Gravitational waves normally propogate at the speed of light.... so what happens when you slow it down?",
	metaNote = "Nom Nom",
	...
	weapon = {
		...
		rangeInUnits = weaponStats.gwe.baseRange,            -- Target must be within range, for gun to fire.
		...
		damageData = {
			...
			impulseForce = weaponStats.gwe.impulse,                 -- float: Apply this much push force. This is reduced by target mass like normal physics.
		},
		...
		--Visuals
		sizeInUnits = 0,         --Controls laser width, 
		lifetime = 0,            --Controls how long the laser lasts,
		impactSize = 0,          --For LASERS, TESLA, controls the size of the impact effect.
		impactDuration = 0,    --Seconds, controls how long the impact effect lasts.
		...
		laserDescription = { 		--"laser" description, but is actually a maleable visual effect.
						...
						noRescaleLength = weaponStats.gwe.baseRange,	--0 means DO rescale like a laser beam. >0 means don't, like a muzzle flashes.
		...
		secondsPerCycle = weaponStats.gwe.baseCD,        --Seconds per reload. Negative value prevent reloading (limited ammo weapons).
		secondsPerCycleCurrent = 0,   --Starting delay. Good if you don't want your bomber launching bombs the moment it spawns.

		--Magazine Size
		shotsPerCycle = weaponStats.gwe.shotsPerBurst,            --Shots per reload, ATS will ensure this number is always at least 1.
		shotsPerCycleCurrent = weaponStats.gwe.shotsPerBurst,     --Starting shots in the clip.
		simultaniousShots = 1,        --How many shots we can make at once. (shotgun, cluster bomb)

		--Fire Rate
		secondsPerShot = 0.1,           --Delay between shots.
	...
	targeting = {
		...
		tracking = {
			...
			invalidationVelocityHeadOn = 2 	--For tractor ships,
		}
	},
}