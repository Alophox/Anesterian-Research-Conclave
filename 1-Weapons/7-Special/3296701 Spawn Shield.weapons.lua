return {
	...
	weapon = {
		...
		rangeInUnits = .02,            -- Target must be within range, for gun to fire.
		maximumAngleToTargetDegrees = 0,   -- Radians, target must be within angle for gun to fire.
		...
		damageData = {
			...
			damage = -.000001,                      -- float: Raw damage, reduced by armour.
			piercing = -99999999,                      -- float: Ignore this much armour. Negative piercing is AEGIS shielding.
		...
		secondsPerCycle = -.2,        --Seconds per reload. Negative value prevent reloading (limited ammo weapons).
		secondsPerCycleCurrent = .1,   --Starting delay. Good if you don't want your bomber launching bombs the moment it spawns.
	...
	targeting = {
		acquisition = {
			...
			secondsPerScan = .01, 			--How long to wait before reassessing what you're targeting.
