...
return {
	id = 3295710,
	name = "Electromagnetic Sensor Suite",
	blurb = "Reveals cloaked ships. While radar is known to be ineffective against modern cloaking technologies, it's not the only range of wavelengths of light.",
	metaNote = "",
	...
	weapon = {
		...
		rangeInUnits = weaponStats.sensor.baseRange * weaponStats.rangeMult[weaponConst.size],            -- Target must be within range, for gun to fire.
		maximumAngleToTargetDegrees = 0.57295779513082323,   -- Radians, target must be within angle for gun to fire.
		...
		damageData = {
			...
			damage = weaponStats.sensor.baseDMG * weaponStats.damageMult[weaponConst.size],                      -- float: Raw damage, reduced by armour.
			piercing = weaponStats.sensor.basePierce,                      -- float: Ignore this much armour. Negative piercing is AEGIS shielding.
			shred = weaponStats.sensor.baseShred / weaponStats.sensor.instances,                      -- float: Destroy this much armour * class shred resistance. Negative heals armour.
			...
			decloak = true,                     -- float: Reduce target cloak by this amount. Unused.
		...
		aoeData = {
			...
			radiusOuter = weaponStats.sensor.baseAOE,         --Damage drops off to 0 at outer. Linear scale. A tolly is 0.4 units
			radiusInner = weaponStats.sensor.baseAOE,          --Full damage to units within inner.
		},

		--Audio
		sfxID = 0,               --int, id of the audio to play when this weapon fires.
		SFXIntensity = 0,      --float, controls how far the sound reaches, and how important it is.

		--Visuals
		sizeInUnits = .05,         --Controls laser width, 
		lifetime = .3,            --Controls how long the laser lasts,
		...
		secondsPerCycle = weaponStats.sensor.baseCD,       --Seconds to reload a full magazine, or reloadAmount if it's not 0.
		secondsPerCycleCurrent = 0,   --Starting delay. Good if you don't want your bomber launching bombs the moment it spawns.

		--Magazine Size
		shotsPerCycle = weaponStats.sensor.shotsPerBurst,            --Shots per reload, ATS will ensure this number is always at least 1.
		shotsPerCycleCurrent = weaponStats.sensor.shotsPerBurst,     --Starting shots in the clip.
		simultaniousShots = 1,        --How many shots we can make at once. (shotgun, cluster bomb)

		--Fire Rate
		secondsPerShot = weaponStats.sensor.secondsPerShot,           --Delay between shots.
	...
	targeting = {
		acquisition = {
			...
			maximumDistance = weaponStats.sensor.baseRange * weaponStats.rangeMult[weaponConst.size], 			--Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
