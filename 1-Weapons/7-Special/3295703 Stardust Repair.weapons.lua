...
return {
	id = 3295703,
	name = "Repair",
	blurb = "Repairs hull and increases aegis regen of nearby allies.",
	...
	weapon = {
		...
		rangeInUnits = weaponStats.stardust.baseRange,            -- Target must be within range, for gun to fire.
		...
		damageData = {
			...
			shred = weaponStats.stardust.baseShred,                      -- float: Destroy this much armour * class shred resistance. Negative heals armour.
		...
		aoeData = {
			...
			radiusInner = weaponStats.stardust.baseAOE,          --Full damage to units within inner.
		},
		...
		secondsPerCycle = weaponStats.stardust.baseCD,       --Seconds to reload a full magazine, or reloadAmount if it's not 0.
		secondsPerCycleCurrent = 0,   --Starting delay. Good if you don't want your bomber launching bombs the moment it spawns.

		--Magazine Size
		shotsPerCycle = weaponStats.stardust.shotsPerBurst,            --Shots per reload, ATS will ensure this number is always at least 1.
		shotsPerCycleCurrent = weaponStats.stardust.shotsPerBurst,     --Starting shots in the clip.
		simultaniousShots = 1,        --How many shots we can make at once. (shotgun, cluster bomb)

		--Fire Rate
		secondsPerShot = weaponStats.stardust.secondsPerShot,           --Delay between shots.
	...
	targeting = {
		acquisition = {
			...
			maximumDistance = weaponStats.stardust.baseRange, 			--Important. Scan radius. Don't target things further than this. KEEP THIS NUMBER LOW, SERIOUS PERFORMANCE IMPACT. Light ~15, Medium ~20, Heavy ~25, Capital ~25
